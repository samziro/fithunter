...
const PackageCard: React.FC<PackageCardProps> = ({
  ...
  return (
    <div className={`${base} ${variantClass}`}>
      ...
      <h3 className="text-xl md:text-2xl font-bold mb-2 text-textSecondary font-poppins">{title}</h3>
      ...
      <button onClick={onClick} className={`${ctaClass} px-6 py-3 font-semibold block w-full transition-colors font-poppins`}>{ctaLabel}</button>
    </div>
  );
};
